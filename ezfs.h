...
/* In the VFS superblock, we store pointers to the pages for the superblock
 * and the inode store so that we can easily read from them and mark them
 * as dirty when they're modified.
 */
struct ezfs_sb_pages
{
	struct page *sb_page;	  /* Points to the superblock page */
	struct page *istore_page; /* Points to the istore page */
};
